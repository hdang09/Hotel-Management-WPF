...
namespace TranHaiDangWPF
{
    ...
    public partial class BookingForm : Window
    {
        ...
        {
            InitializeComponent();
            _bookingService = new BookingHistoryService();
        ...
        private void btnAdd_Click(object sender, RoutedEventArgs e)
        {
            if (dgAvailableRooms.SelectedItems.Count > 0)
            {
                room = new RoomDTO()
                {
                    RoomId = ((RoomInformation)dgAvailableRooms.SelectedItem).RoomId,
                    RoomNumber = ((RoomInformation)dgAvailableRooms.SelectedItem).RoomNumber,
                    RoomMaxCapacity = ((RoomInformation)dgAvailableRooms.SelectedItem).RoomMaxCapacity,
                    RoomPricePerDay = ((RoomInformation)dgAvailableRooms.SelectedItem).RoomPricePerDay,
                };
